...
//spin -B -T gpu_pipeline_frame_voodoo.pml 

#define VERTEX_SHADERS 0  // 3dfx Voodoo 3 (T&L by CPU) 
...
chan vertex_processed = [20] of { int };      // теперь сюда отправляют VertexFetcher'ы
...
proctype CPU() {
    ...
    for (frame : 1 .. TOTAL_FRAMES) {
        ...
        transform_vertices(angle); //(T&L on CPU)
    ...
    :: simulation_active == 1 ->
        ...
        :: frame_in_progress == 1 && current_working_frame == current_frame ->
            ...
            :: vertex_to_fetch?[vid] ->
                ...
                :: else -> break
                od;
                // Вершина уже преобразована CPU, отправляем индекс прямо в ассемблер
    ...
    :: simulation_active == 1 ->
        ...
        :: frame_in_progress == 1 && current_working_frame == current_frame ->
            ...
            :: vertex_processed?[vid] ->
                vertex_processed?vid;
                if
                :: vertex_ready[vid] == 2 ->   // вершина должна быть готова
...
init {
    #ifdef DEBUG
    printf("\n=== INITIALIZATION OF 3DFX VOODOO PIPELINE VARIANT ===\n");
    printf("GPU Configuration: %d VS (no T&L), %d PS, %d TMU, %d ROP\n",
